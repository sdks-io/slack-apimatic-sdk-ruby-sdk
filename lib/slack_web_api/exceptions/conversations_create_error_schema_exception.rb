# slack_web_api
#
# This file was automatically generated by APIMATIC v3.0 (
# https://www.apimatic.io ).

module SlackWebApi
  # Schema for error response from conversations.create method
  class ConversationsCreateErrorSchemaException < APIException
    SKIP = Object.new
    private_constant :SKIP

    # Note: PHP callstack is only visible in dev/qa
    # @return [String]
    attr_accessor :callstack

    # Note: PHP callstack is only visible in dev/qa
    # @return [String]
    attr_accessor :detail

    # Note: PHP callstack is only visible in dev/qa
    # @return [ConversationsCreateErrorEnum]
    attr_accessor :error

    # Note: PHP callstack is only visible in dev/qa
    # @return [String]
    attr_accessor :needed

    # Note: PHP callstack is only visible in dev/qa
    # @return [String]
    attr_reader :ok

    # Note: PHP callstack is only visible in dev/qa
    # @return [String]
    attr_accessor :provided

    # The constructor.
    # @param [String] reason The reason for raising an exception.
    # @param [HttpResponse] response The HttpReponse of the API call.
    def initialize(reason, response)
      super(reason, response)
      hash = APIHelper.json_deserialize(@response.raw_body)
      unbox(hash)
    end

    # Populates this object by extracting properties from a hash.
    # @param [Hash] hash The deserialized response sent by the server in the
    # response body.
    def unbox(hash)
      return nil unless hash

      @callstack = hash.key?('callstack') ? hash['callstack'] : SKIP
      @detail = hash.key?('detail') ? hash['detail'] : SKIP
      @error = hash.key?('error') ? hash['error'] : nil
      @needed = hash.key?('needed') ? hash['needed'] : SKIP
      @provided = hash.key?('provided') ? hash['provided'] : SKIP
    end

    # Provides a human-readable string representation of the object.
    def to_s
      class_name = self.class.name.split('::').last
      "<#{class_name} callstack: #{@callstack}, detail: #{@detail}, error: #{@error}, needed:"\
      " #{@needed}, ok: #{@ok}, provided: #{@provided}>"
    end

    # Provides a debugging-friendly string with detailed object information.
    def inspect
      class_name = self.class.name.split('::').last
      "<#{class_name} callstack: #{@callstack.inspect}, detail: #{@detail.inspect}, error:"\
      " #{@error.inspect}, needed: #{@needed.inspect}, ok: #{@ok.inspect}, provided:"\
      " #{@provided.inspect}>"
    end
  end
end
